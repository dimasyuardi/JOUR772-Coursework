...
```{r}
# Looking for how many pills were manufactured in Ohio.

ohio_total_pills_manufactured <- total_manufacturers_state(state="OH", key = key)
```

```{r}
# Looking for the average number of pills people received monlty in Baltimore County, Maryland in 2009.

baltimore_county_monthly_pill_average <- summarized_county_monthly(state = "MD", county = "Baltimore", key = key)
```

```{r}
# What pharmacy received the most pills in California.

pharmacy_totals_california <- total_pharmacies_state(state = "CA", key = key)
```
## Analysis
...
```{r}
# What is the pills per person per month for Utah? White women had the highest fertility rate in Utah in 2017 based on CDC data.

utah_annual_pills <- combined_buyer_annual(state = "UT", key = key)
```

```{r}
# What is the annual population for Utah? *Would need to seperate female vs. male.

utah_annual_population <- county_population(state = "UT", key = key)
```

```{r}
# New York and Kentucky have the highest rate for homeless children. What is the pills per person per month for NY? Kentucky?
...
```{r}
# Mississippi has the highest poverty rate. How many pills were shipped into Mississippi?

mississippi_annual_pills <- combined_buyer_annual(state = "MS", key = key)
```

```{r}
# Maryland is one of the states with the lowest fertility rate. How many pills did they receive annually?

maryland_annual_pills <- combined_buyer_annual(state = "MD", key = key)
```

```{r}
# Which county had the highest amount of pills delivered?
...
```{r}
# Data join population total and pills total to get average pills per person.
...
# highest_pills %>%
  # full_join(state_population_per_year, by="buyer_state") %>%
  # mutate(average_pills_person = round((dosage_unit/population_average),2)) %>%
  # arrange(desc(average_pills_person))
```
