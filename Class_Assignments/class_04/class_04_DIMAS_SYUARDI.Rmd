...
# Creating a new object called 'baltimore' by pulling the data .tsv file.
...
# 1. The data has 290,908 rows and 42 columns 2. BUYER_ZIP should not be listed as a 'dbl' since they don't need to be added or calculated. BUYER_DEA_NO can be listed as a 'dbl' if you want to add them up or organize them a certain way, but the labels are pretty accurate for this data set. 3. Missing values in this data set are listed as 'null'
...
# NEWCARE HOME HEALTH SERVICES had the most pills sent to them with 48000 pills

# The DEA and state of Maryland convicted Steven Abiodun Sodipo and Callixtus Onigbo Nwaehiri of illegally selling nearly 10 million pills and failing to file and pay the appropriate taxes.

# Newcare Home Health Services were convicted in selling nearly 10 million pills between 2006 and 2012. A federal judge in Baltimore, Maryland charged Steven Sodipo and Callixtus Nwaehiri for illegal drug sales, tax fraud and conspracy to defraud the IRS.
...
# MCKESSON CORPORATION sent the most pills to Baltimore City with 95,932 shipments and 49,710,290 total pills. The headquraters for MCKESSON CORPORATION is in Maryland.

baltimore %>%
group_by(reporter_name, reporter_state, buyer_city) %>%
summarise(shipments = n(),
total_pills = sum(dosage_unit)) %>%
arrange(desc(total_pills))
...
# Professional Pharmacy Service at the Sinai hospital in Baltimore ordered 6,254 shipments. What is unique about this location is that it is a hospital, so we can assume they had a lot of patients that needed this drug.
...
# RiteAid pharmacy received the highest total percentage of pills with 12.52% or 16,740,590 pills of 133,721,503 total pills sent to Baltimore City.
...
# There are 3 different types of RiteAid in this data set. The things that grouped them differently were different punctuation and additional characters.
...
# There were 133,721,503 total shipments to Baltimore County. 13.14% of that were for RiteAid stores.
...
# Pipeline column created - listing our the "pipeline" from where the pills were being distributed to who was buying them.

baltimore %>%
  mutate(drug_name)%>%
    mutate(pipeline = paste0(reporter_name,"--", buyer_name,"--", combined_labeler_name))
```
```{r}
...
# Using the pipeline information, I would look into which pipeline was the most used and find out if there is any significance to the dosage unit or anything else that might be worthy to question.
...
summarise(shipments = n(),
 total_pills = sum(dosage_unit),
 avg_pills_per_shipment = mean(dosage_unit),
 max_pills_per_shipment = max(dosage_unit)) %>%
 arrange(desc(total_pills))
...
# Q: What were the 3 largest pharmacy orders for NEWCARE HOME HEALTH SERVICES? Who were the distributors?

baltimore %>%
  group_by(buyer_name, reporter_name) %>%
  summarise(pills_per_pharmacy = sum(dosage_unit)) %>%
arrange(desc(pills_per_pharmacy)) %>%
filter(pills_per_pharmacy > 1000000) %>%
 filter(str_detect(buyer_name, "NEWCARE"))

# A: Anda Inc and McKesson Corp were the ones who provided the most pills to NEWCARE with all three orders being over 1,000,000.
...
# Q: How many total pills were bought from NEWCARE HOME HEALTH SERVICES? 
...
# A: 6,133,600 total pills
```

```{r}

# Q: What is the percentage of NEWCARE HOME HEALTH SERVICES pill in relation to total pills from Baltimore City?
...
# Q: Which distributor state delivered the most to NEWCARE HOME HEALTH SERVICES?

baltimore %>%
    group_by(reporter_state, buyer_name) %>%
  summarise(pills_per_pharmacy = sum(dosage_unit)) %>%
  arrange(desc(pills_per_pharmacy)) %>%
 filter(str_detect(buyer_name, "NEWCARE"))

# A: Ohio delivered the most with 2,298,200 pills
