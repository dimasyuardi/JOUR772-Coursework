...
```{r}
# buyer_annual_by_year is probably higher than buyer_total because the year variable adds entries for every specific report in that year.

# buyer_address could be higher than buy_totals because it is one entry for all 6 years.
```
...
#Through some Googling, it looks like the data does match my searches. Charleston, SC had the highest rate of opioid distribution of any other place in the nation.
...
# Mathur, Rakesh K had the most total pills with 337,450 in 2010

# Compared to the next doctor, Balasubramanian, Sriram, Mathur, Rakesh K was 1/3 more that as Balasubramanian only had 212,800.

# Dr. Rakesh K Mathur was licensed on 10/11/89, but now his license status is probation. On February 7, 2012 the Board stated that his practice has a likelihood of risk to public harm based on his prescribing of opioids.

# By reading the known disciplinary actions and matching the name, I was able to conclude that the doctor on the lookup tool matches the one in our data set.
...
# West Virginia had the highest rate of total pills per person with 464 pills.
...
# What the code above is doing is providing me a list of fields that are NOT in buyer_totals by using anti_join. Basically, there are 129,884 rows that buyer_addresses has that buyer_totals does not. Which makes sense since the total number of rows in buyer_addresses is 278,697 compared to 148,597 rows in buyer_totals.
...
# Q: Which pharmacy sent the most pills per person?
...
# A: A retail pharmacy in Charleston, SC sent an average of 113 pills per person.
