...
#Creating a new object 'mingo' by pulling from the .tsv file
...
# The most significant combo is Miami-Luken > Tug Valley Pharmacy, LLC > Actavis Pharma, Inc.- with 4,954,200 pills

mingo %>%
  group_by(reporter_name, buyer_name, combined_labeler_name) %>%
  summarise(pills_per_grouping = sum(dosage_unit))%>%
...
```{r}
mingo %>%
  group_by(reporter_name, buyer_name, combined_labeler_name, reporter_dea_no, reporter_city, reporter_address1) %>%
  summarise(pills_per_grouping = sum(dosage_unit))%>%
  arrange(desc(pills_per_grouping))
```
...
What I would search into Google to help me find this answer is:

1. How to filter only data with the word 'Cardinal' in it so I can see all the data we are working with.

2. How to do find and replace in R for characters in data set.

3. How to combine all data with 'Cardinal' in it.
